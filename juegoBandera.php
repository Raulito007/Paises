...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['iniciar'])) {
    ...
    header('Location: juegoBandera.php');
    exit;
}
...
if ($estadoPartida['terminada']) {
    ...
    if ($continente === 'todos') {
        $result = $conn->query("SELECT nombre_del_pais, capital, continente, bandera FROM paises_del_mundo WHERE nombre_del_pais NOT IN ('nombre_del_pais', 'capital', 'continente') AND bandera IS NOT NULL AND bandera != ''");
    } else {
        $result = $conn->query("SELECT nombre_del_pais, capital, continente, bandera FROM paises_del_mundo WHERE continente = '$continente' AND nombre_del_pais NOT IN ('nombre_del_pais', 'capital', 'continente') AND bandera IS NOT NULL AND bandera != ''");
    }
...
<body class="bg-light" style="background-color: #6c757d;">
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0"><img src="icons/bola.png" alt="" class="bola-titulo">🏳️ Juego de Banderas</h1>
        ...
        <div class="pregunta-box">
            <h2 class="text-center">🏳️ Bienvenido al Juego de Banderas</h2>
            <p class="text-center">Identifica los países por sus banderas.</p>
            <p class="text-center mb-4"><strong>Tipo de preguntas:</strong></p>
            <ul>
                <li>¿A qué país pertenece esta bandera?</li>
            </ul>
            <form method="POST" class="text-center">
                <div class="mb-3">
                    <label class="form-label fw-bold d-block" style="color: #333;">Selecciona un continente (o todos):</label>
        ...
        <?php elseif ($mostrarResultado && ($resultado === 'ganado' || $resultado === 'perdido')): ?>
        <div class="pregunta-box">
        ...
        <div class="pregunta-box">
            ...
            <?php if (!empty($paisPrincipal['bandera'])): ?>
            <div class="text-center mb-4">
                <div id="contenedor-bandera">
                    <img src="<?= htmlspecialchars($paisPrincipal['bandera']) ?>" alt="Bandera" style="width: 200px;">
                </div>
            </div>
            <?php endif; ?>
